...
- **#64 — IMPLEMENTED, post-impl review running (rev64impl).** Converted 38 American provinces' `trade_goods=` from generic grain/livestock (+ 2 misplaced New Mexico potato → maize) to the 5 NW crops across 13 setup/provinces/*.txt files, via a deterministic assert-heavy script (BOM+LF preserved, each ID converted once, every conversion logged). American totals now: maize 18, peanut 6, chili 8, sweet_potato 5, potato 6 (Andean) — all floors met, China now minority for maize/peanut/chili. diffstat = 38 one-line swaps, no EOL churn, braces intact, no differentiated good touched. Pending rev64impl verdict → fix if needed → re-review → commit. Then #62 unblocks.
