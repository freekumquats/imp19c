...
## [#233/#240] Italy 1763 map-surgery delta (1763_bookmark)

**Scope.** Second Phase-2 region. Restores the 1763 Italian political map: a sovereign Republic of Genoa,
independent central-Italian duchies, and the full Venetian Stato da Màr; plus the Italy-region unborn-ruler
sweep (#240). All in `setup/main/00_default.txt` (LF/no-BOM) and `setup/characters/00_Italy.txt` (CRLF+BOM).

**New / changed tags & provinces (00_default.txt).**
- **GEN created** (Repubblica di Genova): `oligarchic_republic`, north_italian, catholic, capital 5494.
  own_control_core = Liguria (5494 Genoa / 6103 Savona / 6416 Imperia, **moved from SAR**) + Corsica
  (357 Bastia / 2910 Ajaccio / 4636 Sartène / 9042 Corte, **moved from FRA** — France annexed Corsica only
  in 1768). **LIMITATION:** the Corsican Republic (Paoli, 1755–69) held the interior; with no Corsican tag,
  Corsica is modeled de jure Genoese (documented in-block).
- **Sovereign duchies restored:** removed `AUS→MOD`, `AUS→LUC`, `AUS→PRM`, `AUS→MSS` client_state deps
  (Este Modena / Bourbon Parma / Cybo-Malaspina Massa / Republic of Lucca were all independent in 1763).
  **KEPT** `AUS→TUS` (Habsburg-Lorraine Grand Duchy under Francis Stephen, also HRE Emperor) and `AUS→LBV`
  royal_union (Austrian Duchy of Milan). **LUC** gov `absolute_duchy`→`oligarchic_republic`.
- **Venetian Stato da Màr:** moved **9 provinces AUS→VEN** — 6214 Zara, 8236 Spalato, 4921 Knin, 8375
  Makarska, 10714 Primošten, 1608 Curzola, 10538 Korčula, 181 Kotor/Cattaro, 6685 Pola. VEN now 22 provinces
  (13 terraferma + 9 maritime). **Kept Habsburg (correctly AUS):** 10105 Trieste, 8561 Gorizia, 8687 Postojna
  (Austrian Littoral / Inner Carniola). **LIMITATION:** 1596 Ragusa (sovereign Republic, Ottoman tributary)
  has no tag → left under AUS with an in-block comment rather than orphaned; flagged for a future tag.

**Unborn-ruler sweep (#240, 00_Italy.txt).** Removed `set_as_ruler` wrappers (char defs KEPT) for 4
Napoleonic-era rulers unborn at 1763.2.16: PRM 136 Maria Louisa (b.1791), MOD 139 Francesco IV (b.1779),
LUC 154 Maria Luisa (b.1782), TUS 157 Ferdinand III (b.1769). Period-appropriate rulers 18/110/134/158
(born ≤1763) untouched.

**Process gotchas caught pre-commit.** (a) 00_Italy.txt is **CRLF**+BOM (unlike LF Americas char files); a
first edit pass silently matched nothing until the search blocks used `\r\n` — bytes rewritten with BOM+CRLF
preserved. (b) An early inline-Corsica removal via `re.sub(count=1)` matched a `357 = {` province-*history*
block instead of the FRA core list, corrupting 4 province defs — reverted via `git checkout` and redone with
exact whole-line string targeting. Both verified fixed.

**Verification.** Regression grep for GEN/VEN/MIL/LBV/ION/MOD/LUC/PRM/MSS/TUS/SAR/FRA/AUS + all 16 moved/new
province IDs across events/ + missions/: only `introduction_events.txt` references LUC/PRM/MOD/MSS/LBV, all
pure player-intro flavor gated on `tag = X` with no overlord/subject assumption → no regression. Programmatic
double-ownership audit across all country own_control_core lists: **each moved/new province in exactly one
...
## #314 boot-test fix batch, part 3 (2026-07-10) — B10 Edicts reorg, B25 seclusion, B26 sinicization report, B1/B2/B12 ruler data

**B10 — Grand Council Edicts strip reorganised.** The 26-button strip was cluttered. Now: the two Canton buttons (open/close) collapse into ONE "Canton Trade" button opening a new qing_canton_window (holds both actions, each self-gated); the 4 Court (GBR/FRA/RUS) + 2 Japan (accord/rivalry) buttons MOVED into the Great Game panel (new "Courting the Powers" section, qing_greatgame.gui — the panel body is now wrapped in a scrollarea so it can't overflow); the 3 report buttons (crops/tension/migration) collapse into ONE "Reports" button opening a new qing_reports_window hub (4 launchers incl. the new sinicization report). Great Game panel-opener + marriage buttons kept. New windows in gui/qing_province_reports.gui; loc in qing_governance_l_english.yml + qing_greatgame_l_english.yml.

**B25 — Emperor "retreat into seclusion".** A capable adult emperor can voluntarily withdraw from personal rule (退居休養 / the 太上皇 precedent), opening a Grand Regency. QING_seat_regency_warranted now also true on a new qing_emperor_secluded var; QING_emperor_enter_seclusion (se_QING_SEATS.txt) sets it (-10 legitimacy) and fires the ordinary qing_regency.1 candidate-selection; QING_emperor_resume_rule clears it (+5 legitimacy) and re-evaluates so the regency lapses via qing_regency.3 — UNLESS another warrant (child/incapable/aged) still holds. Two scripted_guis (seclusion shown only to a capable adult non-aged emperor; resume shown while secluded), wired into the Edicts strip. Review confirmed the off-pulse QING_seat_evaluate_regency call is safe and the mutual-exclusion with auto-regency is correct.

**B26 — Sinicization report.** New qing_report_open_sinicization: lists provinces whose dominant_province_culture.culture_group = chinese_group (the concrete 漢化 footprint). Uses the proven dot-accessor culture-group form (mirrors chinese_language_culture_trigger) + GetDominantCulture.GetName in the window. Added to the B10 Reports hub.

**B1/B2 — systemic 1763 ruler data; B12 cascade.** Root cause: the #240 cleanup removed UNBORN 1815 rulers but left several major powers with NO set ruler, so the engine generated random placeholders (the "Grigoriy Bulganin" the user saw for Russia) — and those placeholders spawn CHILDLESS, which is why the marriage candidate table was empty (B12). Plus a few powers still seated children. A research agent identified the correct in-file 1763 ruler for each affected roster. Fixes:
  - FRA: ruler moved from Talleyrand (char:0, age 9) → Louis XV (char:406, the reigning king).
  - RUS: seated Catherine II (char:144, alive 1763, heads the Romanovs) — was engine-gen.
  - GBR: seated George III (char:87, the actual 1760-1820 king) AND fixed his traits — he carried lunatic/incapable/stressed (his 1815 Regency-crisis state); at 1763 he was 25 and sane, so replaced with confident/just + restored finesse/charisma.
  - USA: removed set_as_ruler/coruler from James Madison (char:71, age 12 in 1763 + USA is a pre-independence anachronism) → engine-gen placeholder.
  - TUR: seated Abdul Hamid I (char:42, the only adult Osmanlı prince in-file; the real 1763 sultan Mustafa III isn't in the roster) — was leaving the throne to a random placeholder/toddler.
  - Left as-is (historically CORRECT child rulers per research): Naples Ferdinand IV, Sardinia Vittorio Emanuele I, etc. Left to engine-gen (no in-file candidate): SWE/PRU/BAV/SXH.
...
## #348 diplo_play_fix — LAND_transfer_provinces "Unknown trigger type: list" (breaks ALL land transfers) (2026-07-11)

**Symptom:** boot error.log — "Unknown trigger type: list, near line 24" x38 (one per call site), all traced through `LAND_transfer_provinces` (common/scripted_effects/se_LAND.txt). This effect is the ONLY path for land transfers, so the parse failure broke every diplomatic-play resolution (get_territory/annex/purchase/liberate/colonise), AI peace deals, and Qing subject absorption/integration.

**Root cause (empirically proven, not theorised):** an `any_in_list` TRIGGER does not accept a MACRO-substituted `list = $target_provinces$` — Clausewitz resolves trigger list-args at parse time, BEFORE macro expansion, so the `$target_provinces$` token is unresolved and `list` is read as an unknown trigger. The broken construct was `if = { limit = { any_in_list = { list = $target_provinces$  state = { has_variable = DIPLOMACY_influenced_by } } } ... }`, added in the recent upstream refactor 554d8875 ("Refactored add and remove state influence, also accounting for land transfer") — it shipped broken; the 1763 boot caught it.
  - PROVEN via oracle + repo: NO `any_in_list = { list = $macro$ }` works anywhere in the repo OR the Invictus oracle. Contrast: se_DIPLOMACY.txt:925 `any_in_list = { list = play_cede_candidates count>=1 }` (LITERAL list) does NOT error; the sibling `every_in_list = { list = $target_provinces$ }` and `ordered_in_list = { list = $list$ }` (EFFECT iterators, resolve their list at runtime) and the bare shorthand `is_in_list = $target_provinces$` all handle the same macro fine. The failing pattern is narrow: the `{ list = $macro$ }` BLOCK form of a TRIGGER (any_in_list) specifically.
