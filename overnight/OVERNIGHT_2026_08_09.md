...
## Queued boot-bug fixes + trade-economy program (2026-08-09 late)

Cleared the queued boot-test bugs the user asked for ("do these"), each code-reviewed + committed + pushed, authored freekumquats:
- **#34** (a058b2e07) — Personnel "Clash" (qing_personnel.2): D3 raise clash threshold 40→55 (base-20+culture-25=45 was auto-clashing every Han governor under a Manchu minister); D1 per-governor cooldown qing_personnel_clashed_recently (730d); D2 hand the flagged governor to the event via qing_personnel_clash_target instead of re-drawing. Reviewed SOUND (per-governor cooldown scope confirmed).
- **#41** (ed867f27c) — frontier turf-war (qing_frontier.1) joins the shared court slot (qing_gc_event_slot_used) so it enters the once-per-quarter variety rotation; + on-ladder gate on both SUBJ_QING_advance_integration calls (folds the #19-review LOG_fail for garrisoned Tibet/Kobdo). Reviewed SOUND.
- **#42** (2b1657c1d) — 戶部銀庫 silver-reserve panel now shows the TRUE quarter-over-quarter change: a mod-own snapshot delta (qing_silver_reserve_change_last) computed before this quarter's accumulation, capturing all writers, displayed signed on the Revenue panel. Fixes "reserve rises while (change) shows negative." Reviewed SOUND.
