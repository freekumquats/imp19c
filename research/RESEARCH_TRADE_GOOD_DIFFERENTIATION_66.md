...
- **peanut, chili**: luxury-base fallback + a PARTIAL food role — **[CORRECTION, rev-65 2026-08-10]** they are NOT "luxury-only": DEMAND_food_svalues.txt:101-102 already ADDS GOODS_governorship_peanut_produced + _chili_produced to DEMAND_fulfilled_food_need_governorship (the "quarter-food" tier, divide=2 at :108), and both are in the capacity sweep (se_QING_COLON.txt:296-297) + diffusion snapshot (:418-419) identically to the other 3. The ONLY gap vs maize/potato/sweet_potato is membership in the DYNAMIC food BASKET / DEMAND_num_food_goods (#279 divisor) — a narrow #62 demand-svalue decision. So they carry LESS differentiation than the other 3 but are NOT dead weight. (The original "luxury-base ONLY, no food path" line was stale.)
...
- peanut + chili sit at a lower differentiation tier (luxury + partial food, no basket membership, no building/BOM). **[rev-65 CORRECTION]** the BOM-hookup idea (peanut→processed_foods, chili→pharmaceuticals) is BUILDABLE but INERT: the industrial-BOM demand path is DISABLED (se_DEMAND.txt:6-9) + gated on debug-only INDUSTRY_factories_assigned_* vars (memory imp19c-manufactured-goods-risk / #133) — so it delivers zero demand in the agrarian-boom era. => do NOT use BOM to differentiate them. The real (narrow) lever is a #62 food-BASKET-membership decision, NOT a #65 building. #65's deliverable is a generic farmstead + a mission beat (see DESIGN_NWCROP_POPBOOM_65 §CORRECTIONS), not a peanut/chili hookup.
