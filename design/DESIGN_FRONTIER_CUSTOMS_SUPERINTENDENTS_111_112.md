...
## 10. #112b/#112c IMPLEMENTATION DESIGN (2026-08-10) — spikes CONFIRMED, aqsaqal + contest

**BOTH boot-spikes are now CONFIRMED (Aug-9 22:42 debug.log), so #112b/#112c are unblocked:**
- SPIKE 1 (#26, PASSED): "AQSAQAL SPIKE ok: created a Kokandi resident and his employer is c:KOK" + "persist ok: alive and still on c:KOK". => create_character + move_country onto the INDEPENDENT non-subject foreign c:KOK works AND persists across ticks. The teardown paths (death + KOK-annexed) are already written in se_QING_AQSAQAL_SPIKE.txt (QING_aqsaqal_spike_teardown / _teardown_kok_lost) — LIFT them into the real feature.
- SPIKE 2 (#27, CONFIRMED by construction + fired 13+×): qing_super_contest_chance_svalue = 50 + super(finesse+charisma)×2 − aqsaqal(finesse+zeal)×2, clamped 10..90; qing_super_contest_fail_svalue = 100 − chance. Sum = 100 by definition. => the two-sided win-prob render is proven; LIFT the svalue pair into real events.

**REVISED post-review (rev-112bc §10-R corrections folded into the body below; round-2 review pending). Round-1 verdict: PROCEED-WITH-CORRECTIONS, with C1/C2 as build blockers for #112b and H2/H3 for #112c. Round-1 findings preserved verbatim in §10-R at the bottom as an audit trail.**

### #112b — the real aqsaqal (promote the spike to a shipping character)
- **Var** `qing_caravan_aqsaqal_holder` on CHI + a new char marker `qing_caravan_aqsaqal_marker` (shipping namespace; off the spike's `qing_aqsaqal_spike_*`). KEEP the `qing_caravan_aqsaqal_granted` FLAG (7 reads gate qing_caravan.1/.2/.3 + the panel — layer the character ON TOP of the flag; do NOT remove the flag). **Seat and teardown are PAIRED at the flag's set/remove sites so flag and character stay in strict lockstep** (rev-112bc C1/C2).
- **Seat him in the shared grant EFFECT, not in an event option** (rev-112bc C1 — the flag has ≥3 grant paths, ALL routing through `QING_caravan_grant_aqsaqal` se_QING_CARAVAN.txt:512: qing_caravan.1.grant events:83, qing_caravan.2.negotiate events:189, the panel lever QING_caravan_panel.txt:116, AND the trade-concession law `qing_frontier_trade_concession` on_enact 00_qing_statutes_laws.txt:607 — the law caller is automatically covered BECAUSE the mint lives in the choke-point effect, the whole point of C1). Mint INSIDE `QING_caravan_grant_aqsaqal` (the single choke point), guarded `exists = c:KOK` AND holder-not-already-set, so every grant path seats exactly one aqsaqal and no path sets the flag without a counterparty. Mint via the SPIKE 1 proven shape (create_character age 40-55, culture uzbek, religion sunni, finesse/martial/charisma/zeal spread; set_as_minor_character THEN move_country = c:KOK; store the CHI link var + marker). NO modifiers inside create_character (#90; runtime effect so #90-safe by placement).
- **Teardown PAIRED into revoke + the death/KOK-lost hooks** (rev-112bc C2/H1 — this is the build gate, NOT "reuse verbatim"): the spike's teardown effects are keyed to the SPIKE marker + called from spike-gated sites, so retiring the spike DELETES them. Author SHIPPING teardown effects keyed to `qing_caravan_aqsaqal_marker`, and wire THREE call sites: (a) `QING_caravan_revoke_aqsaqal` (se_QING_CARAVAN.txt:551 — clears the flag; MUST now also tear down the char, else dangling character + desync + a later re-grant double-mints); (b) a NEW teardown call in on_character_death (marker-guarded, matching the spike's own review-fix shape); (c) the KOK-lost teardown moved into `QING_caravan_pulse` UNGATED (not the debug block). Without (a)/(b)/(c) the promoted feature has NO disband path (lifecycle-symmetry violation).
- **Panel card**: CONVERT the debug aqsaqal-spike card (actual span qing_caravan.gui:223-258, gated on qing_aqsaqal_spike_card) into the real counterparty card gated on a new `qing_caravan_aqsaqal_seated` scripted_gui reading qing_caravan_aqsaqal_holder — the superintendent card (qing_caravan.gui:88-119, qing_caravan_super_holder.GetCharacter) is the exact clone; render the aqsaqal via .GetCharacter, visibly distinct (foreign — Kokand flag/colour, "阿奇木伯克" title). Drop the SPIKE 1 debug card.
- **Retire the spike fully** once promoted (rev-112bc L2 — the checklist is MORE than the .txt + card widget): remove se_QING_AQSAQAL_SPIKE.txt (its qing_super_contest_shown/_fail_shown vars :178-179 ride out with it), the loc key `QING_AQSAQAL_SPIKE_CONTEST` at its DEFINITION `qing_caravan_l_english.yml:21-22` (gui/qing_caravan.gui:255 is only the widget use-site — remove the widget there AND the key in the .yml, else an orphan key survives), the scripted_gui `qing_aqsaqal_spike_card` (QING_caravan_panel.txt:181), the auto-arm `QING_aqsaqal_spike_enable=yes` (qing_mechanics_on_actions.txt:336) + the on_character_death spike call (:394), and the debug block (se_QING_GOVERNANCE.txt:392-399). (The throwaway probe super qing_super_spike_holder has no teardown — test-only orphan; note in commit, L1.)

### #112c — the superintendent↔aqsaqal contest events
- **The contest svalues ALREADY SHIP — do NOT lift/duplicate** (rev-112bc M1): `qing_super_contest_chance_svalue`/`_fail_svalue` are already defined in QING_governance_svalues.txt:433-454; the spike only REFERENCES them. #112c just needs the events to `save_scope_as = qing_super` (= qing_caravan_super_holder) / `= qing_aqsaqal` (= qing_caravan_aqsaqal_holder) — the exact scope names the svalues read. (Both read from CHI scope via save_scope_as — proven, NOT the var:X.finesse-reads-0 trap.)
- **N svalues, NOT one branch-parameterized family** (rev-112bc H2 — a plain script_value cannot take a branch parameter selecting which stat to read; the shipping svalue HARDCODES finesse+charisma vs finesse+zeal). Author, IN PLACE in QING_governance_svalues.txt, **3 chance svalues + 3 fail complements**: NEGOTIATION = super(finesse+charisma) vs aqsaqal(finesse+zeal) [the proven spike formula — reuse as-is]; COERCION = super.martial vs aqsaqal.martial (Kokandi-raider threat) [NEW, same clamp shape]; COLLUSION/bribe = weights corruption (venal super / bribable aqsaqal) [NEW, same clamp shape]. Each fail = 100 − its chance. Render "On Success (X%)" / "(100−X%)" via the stash-and-read idiom (qing_amban_negotiate_chance_shown precedent — the render path is PROVEN: se_SUBJECT_QING.txt:854 sets the _shown var, loc reads the same name).
- **Guard both chars exist+alive before save_scope_as** (rev-112bc M3): the super is always seated, but the aqsaqal exists only post-grant (1820s+) and either can be torn down — gate every contest beat on `has_variable=qing_caravan_aqsaqal_holder` + both `is_alive=yes` before saving scopes, else the svalue gets a broken scope.
- **Events**: update qing_caravan.1/.2/.3 to read the two real characters (they currently read the flag only); add the §7 beats (concession negotiation, collusion/extortion, competent-super commendation, Kokand-pressure escalation → ties the aqsaqal to the khoja-invasion chain). CHI-player-gated; **court-slot + dept cooldown are MANDATORY, not "if needed"** (rev-112bc H3 — the caravan pulse today sets NO qing_gc_event_slot_used + has no dept cooldown, so adding beats without them reintroduces the #41 flood): bring the new beats (ideally the existing .1/.2/.3 offers too) under `NOT={has_variable=qing_gc_event_slot_used}` + set_variable, PLUS a new `qing_dept_cd_caravan` days=270 cooldown on the proven n_canton pattern (se_QING_CANTON.txt). Percentages-shown; loc; NO macro-in-LOG.
- **#12 (caravan↔aqsaqal coupling) requires GENUINE bidirectional coupling** (rev-112bc M2): #12 is satisfied ONLY if each rewritten beat both (a) READS the two chars' stats into the odds AND (b) WRITES an outcome back to char state (aqsaqal loyalty/opinion, super corruption) — spec at least one outcome-side write per beat. A render-two-portraits-and-odds rewrite is one-directional and does NOT satisfy #12.

### Build order (locked): #112b (aqsaqal char seated in the shared grant effect + card + teardown paired into revoke/death/KOK-lost, retire spike fully) → #112c (3 chance+3 fail svalues in place + contest events + mandatory slot/dept throttle + #12 write-back). Build gate: C1/C2 before #112b, H2/H3 before #112c code. Each: impl → adversarial review → fix → re-review clean → commit. se_/events no-BOM/LF; gui/loc BOM. freekumquats/merge-overnight.

## 10-R. ADVERSARIAL DESIGN-REVIEW CORRECTIONS (rev-112bc, 2026-08-10) — PROCEED-WITH-CORRECTIONS
Source grounding solid; both spikes de-risk the hard capabilities. But §10's seating/teardown plan has two flag↔character desync BLOCKERS + three wiring gaps. These supersede the conflicting §10 text. **BUILD GATE: C1 + C2 block #112b; H2 + H3 must be resolved in the #112c spec before its code.**

**C1 [CRITICAL] — seat the aqsaqal in the shared grant EFFECT, not in qing_caravan.1.** The flag has THREE grant paths, all through `QING_caravan_grant_aqsaqal` (se_QING_CARAVAN.txt:512): qing_caravan.1.grant (events:83), qing_caravan.2.negotiate (:189), AND the panel lever. §10:461 ("seat when qing_caravan.1 sets the flag") is wrong — granting via .2 or the panel would set the flag with NO aqsaqal minted → the §9b-correction-2 desync. => mint INSIDE QING_caravan_grant_aqsaqal (the single choke point), guarded `exists = c:KOK` AND holder-not-already-set. #90-safe (runtime effect).

**C2 [CRITICAL] — the REVOKE path is unaddressed → dangling character.** `QING_caravan_revoke_aqsaqal` (se_QING_CARAVAN.txt:551) clears the flag but §10 never tears down the aqsaqal → flag/char desync + a raised on-map object with no disband path (lifecycle-symmetry violation); a later re-grant double-mints. => revoke MUST run the teardown; seat + teardown PAIRED at the flag's set/remove sites so flag and character stay in strict lockstep.

**H1 [HIGH] — teardown "reuse verbatim" is hand-waved (FINDING #1); retiring the spike deletes the effects AND their call sites.** The death teardown is called from on_character_death (qing_mechanics_on_actions.txt:394) keyed to the SPIKE marker; the KOK-lost teardown is called from the pulse INSIDE the debug-gated block (se_QING_GOVERNANCE.txt:396). Retiring the spike removes both. => specify explicitly: (a) shipping teardown effects keyed to a NEW `qing_caravan_aqsaqal_marker`; (b) a NEW teardown call in on_character_death (marker-guarded); (c) the KOK-lost teardown moved into QING_caravan_pulse UNGATED (not the debug block). Without (b)/(c) the promoted feature has NO death/annex disband path. This + C2 are the build gate.

**H2 [HIGH] — "one svalue family, branch-parameterized" is NOT implementable (FINDING #5); need N svalues.** A plain script_value cannot take a branch parameter selecting which stat to read. => spec **3 chance svalues + 3 fail svalues** (negotiation / coercion martial-vs-martial / collusion corruption), each render-checked. SPIKE 2 only proved the NEGOTIATION formula; coercion/collusion are NEW (same clamp shape, low risk, but not "proven"). Drop the "one family branch-parameterized" language (§10:468).

**H3 [HIGH] — court-slot + dept cooldown is MANDATORY, not "if needed" (#41-class flood).** The caravan pulse today sets NO qing_gc_event_slot_used + has NO dept cooldown; offer gates fire on is_ai=no + state only (se_QING_CARAVAN.txt:349-394). Adding contest beats without a slot guard reintroduces the #41 bug. => MANDATORY: bring the new contest beats (ideally the existing .1/.2/.3 offers too) under `NOT={has_variable=qing_gc_event_slot_used}` + set_variable, PLUS an `qing_dept_cd_caravan` days=270 cooldown on the proven n_canton pattern (se_QING_CANTON.txt). Name the var in the design.

**M1 [MED] — the contest svalues ALREADY SHIP; do NOT "lift" them.** `qing_super_contest_chance_svalue`/`_fail_svalue` are already defined in QING_governance_svalues.txt:433-454; the spike only REFERENCES them. §10:467 "lift into QING_governance_svalues.txt" would no-op or DUPLICATE. => they stay put; #112c only needs the events to `save_scope_as = qing_super` / `= qing_aqsaqal` (the exact scope names the svalue reads). (Extend to the 3-svalue set per H2, in place.)

**M2 [MED] — #12 "satisfied" requires GENUINE bidirectional coupling.** Marking #12 done needs the rewritten .1/.2/.3 to both (a) read the two chars' stats into the odds AND (b) write outcomes back to char state (aqsaqal loyalty/opinion, super corruption). Spec at least one outcome-side WRITE per beat, else it's one-directional (two portraits + odds ≠ coupling).

**M3 [MED] — guard both chars exist+alive before save_scope_as.** Super is always seated; the aqsaqal exists only post-grant (1820s+) and either can be torn down. => gate every contest beat on `has_variable=qing_caravan_aqsaqal_holder` + both `is_alive=yes` before saving scopes.

**LOW:** L1 qing_super_spike_holder (throwaway probe super) has no teardown — orphaned in an already-armed test save; test-only, note in commit. L2 spike-removal checklist must ALSO drop: loc key QING_AQSAQAL_SPIKE_CONTEST (gui/qing_caravan.gui:255), scripted_gui qing_aqsaqal_spike_card (QING_caravan_panel.txt:181), the auto-arm QING_aqsaqal_spike_enable=yes (qing_mechanics_on_actions.txt:336), the debug block (se_QING_GOVERNANCE.txt:392-399) — not just the .txt + card widget. §10's gui line refs are loose (actual debug card = gui:223-258). L3 conquest paths (dictate/absorb) don't retire the aqsaqal (flag stays granted, internally coherent) — optional. L4 KOK-lost as a quarterly poll leaves ≤1 quarter stale pointer; panel has_variable guard mitigates; acceptable, matches spike.

**CONFIRMED SOUND:** both chars read from CHI scope via save_scope_as (NOT the var:X.finesse-reads-0 trap); the _shown stash-and-render precedent is real + correctly wired (se_SUBJECT_QING.txt:854 + the loc read); KEEP-the-flag layering (§9b corr 2) correct; panel-card clone target (qing_caravan_super_holder.GetCharacter, gui:88-119) right.
