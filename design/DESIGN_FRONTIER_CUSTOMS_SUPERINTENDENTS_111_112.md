...
- **Var** `qing_caravan_aqsaqal_holder` on CHI (rename off the spike's `qing_aqsaqal_spike_holder`; the shipping namespace). KEEP the `qing_caravan_aqsaqal_granted` FLAG (7 reads gate qing_caravan.1/.2/.3 + the panel — layer the character ON TOP of the flag, per §9b correction 2; do NOT remove the flag).
- **Seat him** when the aqsaqal concession is GRANTED (qing_caravan.1 "The Kokand aqsaqal concession" — where the flag is currently set): mint via the SPIKE 1 proven shape (create_character age 40-55, culture uzbek, religion sunni, finesse/martial/charisma/zeal spread; set_as_minor_character THEN move_country = c:KOK; store the CHI link var). NO modifiers inside create_character (#90). This is an EVENT-option effect (runtime, not construction) so it's #90-safe by placement — but the concession event can fire early, so guard exists=c:KOK (the spike proves the guard).
- **Panel card**: CONVERT the existing debug aqsaqal-spike card (qing_caravan.gui:219-224, gated on qing_aqsaqal_spike_card) into the real counterparty card gated on a new `qing_caravan_aqsaqal_seated` scripted_gui reading qing_caravan_aqsaqal_holder — the superintendent card (qing_caravan.gui:86-111, qing_caravan_super_holder.GetCharacter) is the exact clone; render the aqsaqal via .GetCharacter, visibly distinct (foreign — Kokand flag/colour, "阿奇木伯克" title). Drop the SPIKE 1 debug card.
- **Teardown (lifecycle-symmetry)**: reuse the spike's two teardown effects verbatim (death → clear CHI link; KOK annexed/gone → clear link + retire) on the real holder var. This is REQUIRED (raise a real object → curtail must disband), and it's already written + spike-tested.
- **Retire the spike files** once promoted: se_QING_AQSAQAL_SPIKE.txt + its auto-arm hook (#31) come out when the real feature lands (mirror the #35 verify-then-strip discipline — but here the spike is SUPERSEDED, so remove with the #112b commit, not deferred).

### #112c — the superintendent↔aqsaqal contest events
- **Lift the proven svalue pair** (qing_super_contest_chance_svalue / _fail_svalue) from se_QING_AQSAQAL_SPIKE.txt into QING_governance_svalues.txt as SHIPPING svalues, reading scope:qing_super (= qing_caravan_super_holder) vs scope:qing_aqsaqal (= qing_caravan_aqsaqal_holder). Both scopes saved in the event immediate (both live — super is a CHI courtier, aqsaqal on c:KOK; the spike proves both read).
- **Attribute→branch map (§7, now locked)**: NEGOTIATION branch = super(finesse+charisma) vs aqsaqal(finesse+zeal) [the proven spike formula]; COERCION branch = super.martial vs aqsaqal.martial (Kokandi-raider threat); COLLUSION/bribe branch = weights corruption (a venal super or bribable aqsaqal). Each branch swaps which stats the chance-svalue reads — one svalue family, branch-parameterized, NOT three separate untested formulas. Render "On Success (X%)" / "(100−X%)" via the stash-and-read idiom (qing_amban_negotiate_chance_shown precedent, which #58 confirmed renders in qing_integ.*.tt).
- **Events**: update qing_caravan.1/.2/.3 to read the two real characters (they currently read the flag only); add the beats from §7 (concession negotiation, collusion/extortion, competent-super commendation, Kokand-pressure escalation → ties the aqsaqal to the khoja-invasion chain). CHI-player-gated; court-slot throttled (qing_gc_event_slot_used) AND — per #55 — give the caravan/frontier department its dept-cooldown treatment so these don't starve the rotation (coordinate with the #55 qing_frontier_turfwar_cd / a new qing_dept_cd_caravan if needed). Percentages-shown; loc; NO macro-in-LOG.
- **#12 (caravan↔aqsaqal coupling)** folds in here: qing_caravan.1/.2/.3 (Khoja/Kokand) now bidirectionally read BOTH the superintendent and the aqsaqal characters (was flag-only) — that IS the #12 bidirectional coupling. Mark #12 satisfied by the #112c event rewrite.

### Build order (locked): #112b (aqsaqal char + card + teardown, retire spike files) → #112c (lift svalues + contest events + #12 coupling). Each: impl → adversarial review → commit. se_/events no-BOM/LF; gui/loc BOM. freekumquats/merge-overnight.
