...
**Salt (the #44 priority) — a TAX MARK-UP, not a base-value bump.** Raw salt <1 wen/catty vs gabelle-taxed retail 30-50 wen/catty ⇒ ~30-50× markup (WEAK source — min.news, CAPTCHA-blocked, order-of-magnitude only; corroborated qualitatively by Ho Ping-ti's ~25M-tael/yr Yangzhou merchant-profit claim). DESIGN CONSEQUENCE: the salt price gap is a POLICY artifact (Lianghuai monopoly), not an intrinsic commodity value — so #44 must model it as a tax/mark-up layered on a MODEST raw base, NOT by raising salt's `local_price`/base everywhere salt is produced. This is consistent with routing salt revenue through the Salt Commissioner office + output (#44 R7), not through a bespoke high salt price.
