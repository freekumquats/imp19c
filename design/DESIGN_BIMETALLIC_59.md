...
## LOCKED BUILD ORDER (review-endorsed)
- **Increment 1 (ship + verify FIRST): Tier A valuation anchor, currency-core UNTOUCHED.** intrinsic_price_silver + intrinsic_price_gold = ×15 (CURRENCY_svalues.txt); reserve valuation / sell-reserve / 戶部銀庫 display read it. Do NOT repoint backing_value. Zero sqrt-chain risk, additive, uniform. Fixes the visible "silver>gold looks wrong for reserves" surface ALONE.
- **Increment 2 (SEPARATE, independently-gated commit): Tier B price-lever pull.** post-hoc change_variable on local_price_gold/_silver (PRICE_factor pattern), uniform per-TZ in the type-6 pass; magnitude = clamp(k×(R−r)) with GLOBAL r, STATELESS recompute, deadband-gated, k small, clamp<½ deadband. Gate on the L2 multi-country observation boot. If gbip destabilizes OR any non-CHI economy breaks → revert Increment 2 ONLY; Increment 1 remains the permanent floor.
Lever CORRECTION: post-hoc local_price (NOT demand, NOT backing_value).
