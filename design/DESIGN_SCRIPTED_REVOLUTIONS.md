...
- **Verbs used (all proven):** `add_alliance = c:USA`, `FUNC_declare_war_with_wargoal_province`, `force_white_peace = THIS` (a WAR scope, fetched via `every_current_war = { limit = { any_war_participant = { THIS = c:TAG } } ... }` -- WAR_scripted_guis.txt:1209 / se_AI.txt:2725 / se_FUNC.txt:362-368; [fix, log-triage 2026-09-03] the earlier `force_white_peace = c:TAG` form documented here was itself the bug -- it errors "scope type country, expected war" -- corrected at both call sites in gbr_empire_events.txt), `release_subject = c:USA`. BOM preserved on both .txt and .yml.
