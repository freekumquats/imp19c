...
**NET #62 fix (final, minimal — rev62 MED + rev62impl HIGH folded):** the double-count is maize/potato/sweet_potato (the 3 crops in BOTH baskets). Fix = for EACH of the three, rewrite its DEMAND_<crop> Total svalue to the PRODUCTION-GATED food form above (has_variable DEMAND_food_<crop> → value=svalue, else 0, min=0). This means:
  ...
  - USE the BOUNDED svalue `value = DEMAND_food_<crop>` (rev62 LOW), NOT the raw `add = var:DEMAND_food_<crop>` — the svalue applies the 110%/90% tick-bounding (DEMAND_food_svalues_new.txt:47-60) that the raw var skips; this is the true DEMAND_grain mirror now that the food fold-in is the SOLE source.
  Then remove each from BOTH luxury call-lists (se_DEMAND.txt:618/624/625 + :655/661/662) as dead-writer cleanup. peanut (:626/:663) + chili (:627/:664): LEAVE (luxury-only, not double-counted, no food path needed). NO new DEMAND_food_peanut. Count block already correct (maize/potato/sweet_potato increments exist).
**Files (final):** se_DEMAND.txt (remove 6 luxury call-list lines; **CRLF+BOM — do NOT normalize to LF**) + DEMAND_luxury_svalues.txt (3 Total svalue food-only rewrites: maize :325 / sweet_potato :480 / potato :534; **LF+BOM**). TWO files. NO DEMAND_food_svalues_new.txt change (all 3 food svalues + count increments already exist). NO peanut work.
